    ...
    <tbody>
        @foreach($calendars->chunk(7) as $week)
            ...
            @foreach($week as $calendar)
                @eval($is_today = ($calendar->date == optional($day_calendar)->date))
                @eval($is_sunday = ($calendar->weekday == Weekdays::SUNDAY))
                @eval($is_saturday = ($calendar->weekday == Weekdays::SATURDAY))
                @eval($is_dish_menu = $calendar->dish_menus()->count() > 0)
                @if($month_calendar->contains($calendar))
